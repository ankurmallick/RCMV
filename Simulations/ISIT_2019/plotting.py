...
for i in range(len(alpha_lt)):
    if alpha_lt[i]==1.25:
        ...
        labelstr='LT ('+keys[2]+' = '+str(alpha_lt[i])+')'
        x,y=gettail(latency_lt[i,:])
        plt.plot(x,y,color='black',linestyle=linestyle[linestyle_ctr],label=labelstr)
        linestyle_ctr+=1
...
#y=avgcomp_dlb/10000
#f=1
#plt.plot(x,y,color='black',marker='+',label='DLB')
#plt.annotate('DLB',xy=(x,y),xytext=(x,f*y))
